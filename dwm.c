...
static XCBAtom getatomprop(Client *c, xcb_atom_t prop);
static int getrootptr(int *x, int *y);
static long getstate(XCBWindow w);
static int gettextprop(XCBWindow w, XCBAtom atom, char *text, unsigned int size);
static void grabbuttons(Client *c, int focused);
static void grabkeys(void);
static void incnmaster(const Arg *arg);
static void keypress(xcb_generic_event_t *e);
static void killclient(const Arg *arg);
static void manage(XCBWindow w, XCBWindowAttributes *wa);
...
static int sendevent(Client *c, XCBAtom proto);
...
static Client *wintoclient(XCBWindow w);
static Monitor *wintomon(XCBWindow w);
static int xerror(XCBDisplay *dpy, XCBGenericError *ee);
static int xerrordummy(XCBDisplay *dpy, XCBGenericError *ee);
static int xerrorstart(XCBDisplay *dpy, XCBGenericError *ee);
 ...
 */
static void (*handler[50]) (xcb_generic_event_t *) = {
...
static XCBAtom wmatom[WMLast], netatom[NetLast];
...
configure(Client *c)
{
	...
	XSendEvent(dpy, c->win, False, StructureNotifyMask, (xcb_generic_event_t *)&ce);
}
...
XCBAtom
getatomprop(Client *c, XCBAtom prop)
{
	int di;
	unsigned long dl;
	unsigned char *p = NULL;
	XCBAtom da, atom = None;
...
int
gettextprop(XCBWindow w, XCBAtom atom, char *text, unsigned int size)
{
	char **list = NULL;
	int n;
	XTextProperty name;
...
void
manage(XCBWindow w, XCBWindowAttributes *wa)
{
	Client *c, *t = NULL;
...
setup(void)
{
	...
	/* init screen */
    scr = xcb_aux_get_screen(dpy, screen);
    if(!scr)
    {   die("FATAL: SETUP_CANNOT_GET_SCREEN");
    }
...
updategeom(void)
{
    ...
    extrep = xcb_get_extension_data(dpy, &xid);
    xienabled = !(!extrep || !ext->present);

    if(xienabled)
    {
        xia = xcb_xinerama_is_active(dpy);
        /* let event handler handle a Xinerama error */
        xcb_xinerama_is_active_reply(dpy, xia, NULL);
...
{
	XCBAtom state = getatomprop(c, netatom[NetWMState]);
	XCBAtom wtype = getatomprop(c, netatom[NetWMWindowType]);
...
int
xerror(XCBDisplay *dpy, XCBGenericError *ee)
{
...
int
xerrordummy(XCBDisplay *dpy, XCBGenericError *ee)
{
	return 0;
}
 ...
 * is already running. */
int
xerrorstart(XCBDisplay *dpy, XCBGenericError *ee)
{
